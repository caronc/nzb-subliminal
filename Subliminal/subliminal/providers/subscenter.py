...
from ..exceptions import ProviderAuthenticationError, ProviderConfigurationError, ProviderError
from ..subtitle import Subtitle, compute_guess_matches
...
class SubsCenterSubtitle(Subtitle):
    ...
    def __init__(self, language, hearing_impaired, series, season, episode, title, subtitle_id, subtitle_key,
                 subtitle_version, downloaded, releases):
        super(SubsCenterSubtitle, self).__init__(language, hearing_impaired)
        self.series = series
        self.season = season
    ...
    def get_matches(self, video):
        ...
        if isinstance(video, Episode):
            ...
            for release in self.releases:
                matches |= compute_guess_matches(video, guess_info(release))
        # movie
...
class SubsCenterProvider(Provider):
    ...
    def initialize(self):
        ...
        if self.username is not None and self.password is not None:
            ...
            if r.status_code != 302:
                raise ProviderAuthenticationError(self.username)
    ...
    def _search_url_titles(self, title):
        ...
        if r.history and all([h.status_code == 302 for h in r.history]):
            ...
            soup = bs4.BeautifulSoup(r.content, ['lxml', 'html.parser'])
    ...
    def query(self, title, season=None, episode=None):
        ...
        if season and episode:
            ...
            self.referer = self.server_url + 'subtitle/series/{}/{}/{}/'.format(url_title, season, episode)
        else:
            ...
            self.referer = self.server_url + 'subtitle/movie/{}/'.format(url_title)
            ...
            for quality_data in language_data.values():
                for quality, subtitles_data in quality_data.items():
                    for subtitle_item in subtitles_data.values():
                        ...
                        # otherwise create it
                        subtitle = self.subtitle_class(language, hearing_impaired, title, season, episode,
    ...
    def download_subtitle(self, subtitle):
        ...
        r = self.session.get(url, params=params, headers={'Referer': self.referer}, timeout=10)
        r.raise_for_status()
        ...
        try:
            with zipfile.ZipFile(io.BytesIO(r.content)) as zf:
                ...
                subtitle.content = zf.read(namelist[0])
